...
interface AuthDialogProps {
  ...
  handleGoogleAuthInApp?: () => void; // App内Google登录（GIS弹窗）
        ...
        <div className="relative z-10 p-6">
          <DialogHeader className="pb-4">
            <DialogTitle className="flex items-center justify-center space-x-3 text-xl font-bold">
              <div className={`p-2.5 rounded-xl shadow-lg ${authMode === "login"
          ...
          {authMode !== "reset" && (
            <>
              {shouldShowWechatLogin ? (
                ...
                isAndroid && handleGoogleAuthInApp ? (
                  /* Android: 双选项 - App内登录 / 浏览器登录 */
                  <div className="space-y-2 mb-4">
                    <Button
                      onClick={handleGoogleAuthInApp}
                      variant="outline"
                      disabled={!agreePrivacy}
                      className={`w-full flex items-center justify-center space-x-2 h-11 bg-white dark:bg-white/5 hover:bg-gray-50 dark:hover:bg-white/10 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-white/10 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${!agreePrivacy ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      <svg className="w-5 h-5" viewBox="0 0 24 24">
                    ...
                    <Button
                      ...
                      variant="outline"
                      disabled={!agreePrivacy}
                      className={`w-full flex items-center justify-center space-x-2 h-10 bg-gray-50 dark:bg-white/3 hover:bg-gray-100 dark:hover:bg-white/8 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-white/10 text-sm transition-all duration-300 ${!agreePrivacy ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      ...
                      <span>{isZh ? "使用浏览器登录" : "Sign in with Browser"}</span>
                    </Button>
                  </div>
                ) : (
                  /* 桌面/iOS: 单按钮 */
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <div className="flex flex-col space-y-3 pt-2">
              <Button
                type="submit"
                disabled={!agreePrivacy}
                className={`w-full h-11 font-semibold text-white shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${!agreePrivacy ? 'opacity-50 cursor-not-allowed' : ''} ${authMode === "login"
