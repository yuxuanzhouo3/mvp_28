...
Page({
  ...
  async startWxLogin() {
    ...
    try {
      ...
      if (checkResult && checkResult.exists && checkResult.hasProfile) {
        // 用户已存在且有头像昵称，直接返回 webshell
        console.log('[login] User exists with profile, skipping profile page')
        await this.returnToWebshellDirectly(checkResult, checkResult.userName, checkResult.userAvatar)
      } else if (checkResult && checkResult.token) {
        // 用户已存在但没有头像昵称（或新创建的用户），需要填写资料
        // 注意：此时 code 已被 check API 消耗，必须使用返回的 token
        console.log('[login] User exists but needs profile, redirecting with token')
        await this.redirectToProfile(checkResult)
      } else {
        // check API 调用失败，尝试使用原始 code（可能仍然有效）
